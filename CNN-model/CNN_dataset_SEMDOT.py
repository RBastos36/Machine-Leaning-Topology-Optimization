...
def extrapolate_domain_SEMDOT(domain, rmin=1.5):
    """
    Extrapolate element-wise domain to nodal grid using SEMDOT-style weighted averaging.
    domain: (H, W) numpy array of element-wise densities
    returns: (H+1, W+1) numpy array of node-wise densities
    """
    H, W = domain.shape
    nodal = np.zeros((H+1, W+1))
    weights = np.zeros((H+1, W+1))
        ...
        for j in range(W):
            ...
            for dy in [0, 1]:
                for dx in [0, 1]:
                    n_y = i + dy
                    n_x = j + dx
                    dist = np.sqrt((n_y - ec_y) ** 2 + (n_x - ec_x) ** 2)
                    if dist < rmin:
                        weight = rmin - dist
                        nodal[n_y, n_x] += weight * e_density
                        weights[n_y, n_x] += weight

    # Normalize
    nodal /= np.maximum(weights, 1e-8)
    return nodal
...
class FEMDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        padded_domain = extrapolate_domain_uniform(domain)
        ...
        # Stack outputs into a single 2-channel tensor
        output_data = np.stack([disp_x, disp_y])
...
def calculate_dataset_statistics(hdf5_path, json_split_path, batch_size=32):
    ...
    output_sum = torch.zeros(2)  # 2 output channels
    output_squared_sum = torch.zeros(2)

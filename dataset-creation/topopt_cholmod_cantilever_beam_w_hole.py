...
def topopt(nelx, nely, volfrac, penal, rmin, ft, load_config, void_config):
    ...
    print(f"Void config: {void_config}")

    # Create void mask
    void_mask = create_circular_void_mask(
        nelx, nely,
        void_config['center_x'],
        void_config['center_y'],
        void_config['radius']
    )

    # Initialize dataset if needed
    initialize_dataset('cantilever_dataset_w_hole.h5')

    # Open the HDF5 file at the start and keep it open
    h5file = h5py.File('cantilever_dataset_w_hole.h5', 'a')

    try:
        problem_id = generate_problem_id(nelx, nely, volfrac, rmin, load_config, void_config)
        ...
        if problem_id in h5file['problems']:
            ...
            params.attrs['void_center_x'] = void_config['center_x']
            params.attrs['void_center_y'] = void_config['center_y']
            params.attrs['void_radius'] = void_config['radius']
        ...
        while change > 0.01 and loop < 2000:
            ...
            x[:] = x * void_mask.flatten()  # Apply void mask
            ...
            if ft == 0:
                ...
                xPhys[:] = xPhys * void_mask.flatten()  # Reapply void mask
...
def generate_problem_id(nelx, nely, volfrac, rmin, load_config, void_config):
    ...
    return (f"vf{volfrac:.2f}_"
            ...
            f"void_x{void_config['center_x']:.2f}_"
            f"void_y{void_config['center_y']:.2f}_"
            f"void_r{void_config['radius']:.2f}_"
            f"nelx{nelx}_"
            f"nely{nely}_"
            f"rmin{rmin:.1f}")
...
def create_circular_void_mask(nelx, nely, center_x, center_y, radius):
    """Create a binary mask for a circular void region"""
    # Convert relative positions to element coordinates
    center_x_elem = center_x * nelx
    center_y_elem = center_y * nely

    # Convert relative radius to element units
    radius_elem = radius * min(nelx, nely)

    # Create coordinate matrices
    y, x = np.meshgrid(np.arange(nely), np.arange(nelx))

    # Calculate distances from each element to hole center
    distances = np.sqrt((x - center_x_elem) ** 2 + (y - center_y_elem) ** 2)

    # Create mask (1 outside hole, 0 inside hole)
    mask = (distances > radius_elem).astype(float)
...
if __name__ == "__main__":
    ...
    # Void configuration
    void_config = {
        'center_x': 0.3,  # relative x position (0 to 1)
        'center_y': 0.5,  # relative y position (0 to 1)
        'radius': 0.3 # relative radius (relative to min(nelx, nely))
    }

    xPhys, obj = topopt(nelx, nely, volfrac, penal, rmin, ft, load_config, void_config)